...
export default function EventDetailActions({ eventId, status }: EventDetailActionsProps) {
  ...
  const handleApprove = async () => {
    setProcessing(true)
    setError(null)
    const res = await approveEventAction(eventId, adminNote)
    if (res.success) {
      router.refresh()
    } else {
  ...
  const handleConfirmReject = async () => {
    ...
    setProcessing(true)
    setError(null)
    const res = await rejectEventAction(eventId, rejectReason)
    if (res.success) {
      setRejectMode(false)
      router.refresh()
    } else {
        ...
        <div className="space-y-4 bg-gray-50 p-5 rounded-2xl border border-gray-100">
          <div>
            ...
            <input
              ...
              className="w-full border border-gray-200 bg-white rounded-xl px-3 py-2 text-xs text-text placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-pink-200"
            />
          </div>
