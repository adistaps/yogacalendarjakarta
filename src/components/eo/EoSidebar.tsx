...
const navItems = [
  ...
  { href: '/events', icon: Calendar, label: 'Event Saya' },
  { href: '/attendees', icon: Users, label: 'Peserta' },
  { href: '/ads', icon: Megaphone, label: 'Iklan' },
  { href: '/settings', icon: Settings, label: 'Pengaturan' },
]

interface EoSidebarProps {
  orgName: string
  logoUrl: string | null
}

export default function EoSidebar({ orgName, logoUrl }: EoSidebarProps) {
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="px-6 py-5 border-b border-gray-100">
        <Link href="/" className="flex items-center gap-3 group">
          {logoUrl ? (
            <img src={logoUrl} alt={orgName} className="w-9 h-9 rounded-lg object-cover" />
          ) : (
            <div
          ...
          <div className="min-w-0">
            <p className="text-xs text-text-muted truncate">Portal EO</p>
            <p className="text-sm font-semibold text-text truncate">{orgName}</p>
          </div>
        </Link>
      </div>
  ...
  return (
    <>
      ...
      {/* Mobile Toggle */}
      <button
        className="lg:hidden fixed top-4 left-4 z-50 w-10 h-10 bg-white shadow-md rounded-lg flex items-center justify-center"
